...
function tilelivecopy(srcUri, s3url, options, callback) {
  ...
  queue()
    ...
    .await(function(err, src, dst) {
      function done(err) {
        if (err) {
          if (err.message.indexOf('Failed to parse geojson feature') != -1 || // invalid geojson
              err.message.indexOf('required parameter y is out of range') != -1) { // web mercator out of range
            err.code = 'EINVALID';
          }
        }
