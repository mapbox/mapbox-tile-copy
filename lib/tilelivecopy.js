...
function tilelivecopy(srcUri, s3url, options, callback) {
  ...
  options.type = options.scheme || 'scanline';
  if(['put', 'pyramid', 'scanline', 'list'].indexOf(options.type) === -1) return callback(new Error('Invalid scheme. Must be "put", "pyramid", "scanline", or "list".'));
  var protocol = url.parse(srcUri).protocol;
  if (protocol === 'mbtiles:' && !options.scheme) options.type = 'list';
  if (protocol === 'omnivore:' && !options.scheme) options.type = 'pyramid';
  ...
  queue()
    ...
    .await(function(err, src, dst) {
      ...
      src.getInfo(function(err, info) {
        ...
        options.close = true;
        if (options.type === 'list') options.listStream = src.createZXYStream();
