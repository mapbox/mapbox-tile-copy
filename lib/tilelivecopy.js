...
function tilelivecopy(srcUri, s3url, options, callback) {
  ...
  if (options.bundle === true) {
    src_loader = function(bundle_uri, cb) {
      //bundle_uri = "omnivore:///tmp/unpacker.TEST/states-1.geojson,/tmp/unpacker.TEST/states-2.geojson"
      new tileliveOmivore(bundle_uri, function(err, source) {
  ...
  queue()
    ...
    .await(function(err, src, dst) {
      ...
      src.getInfo(function(err, info) {
        ...
        if (protocol === 'mbtiles:') transforms.push(MigrationStream());
