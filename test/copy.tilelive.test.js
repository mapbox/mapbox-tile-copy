...
test('scheme option overrides default', function(t) {
  var fixture = path.resolve(__dirname, 'fixtures', 'valid.geojson');
  var src = 'omnivore://' + fixture;
  var dst = dsturi('valid.geojson');
  sinon.spy(tilelive, 'copy');

  tileliveCopy(src, dst, { maxzoom: 5, scheme: 'scanline' }, function(err) {
    t.ifError(err, 'copied');
...
test('invalid scheme option errors', function(t) {
  var fixture = path.resolve(__dirname, 'fixtures', 'valid.geojson');
  var src = 'omnivore://' + fixture;
  var dst = dsturi('valid.geojson');
  sinon.spy(tilelive, 'copy');
  ...
  tileliveCopy(src, dst, { maxzoom: 5, scheme: 'not-real' }, function(err) {
    t.ok(err);
    t.equal(err.message, 'Invalid scheme. Must be "pyramid", "scanline", or "list".');
    tilelive.copy.restore();
    t.end();
  });
});
